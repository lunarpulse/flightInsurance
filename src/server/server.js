	...
	return new Promise((resolve, reject) => {
		web3.eth.getAccounts().then(accounts => {
			...
			flightSuretyApp.methods.REGISTRATION_FEE().call().then(fee => {
				...
				accounts.slice(11, 53).forEach(account => {
...
initOracles().then(oracles => {
	...
	flightSuretyApp.events.ProvideOracleResponse({
		fromBlock: "latest"
	...
	flightSuretyApp.events.AirlineFunded({
		fromBlock: 0
	}, function (error, event) {
		if (error) console.log(error)
		console.log(event)
	...
	flightSuretyApp.events.CreditInsuree({
		fromBlock: 0
	}, function (error, event) {
		if (error) console.log(error)
		console.log(event)
